...
통합된 **대칭 중지(Symmetrical Stop)** 명령을 사용하세요:
...
1. 관리 중인 서비스(`termux-services`) 중지 (실행 중인 경우)
2. 고아 프로세스 및 좀비 프로세스 전면 검색 및 종료
3. 잠금 파일(`lock file`) 자동 정리

잠시 후 서비스 모드로 다시 시작하는 것을 권장합니다:

```bash
oa start:sv
```

> **참고**: `oa start:sv`는 이제 ARMv7 초기화 지연을 감안하여 최대 **90초** 동안 대기하며, 18789 포트가 실제로 활성화되었는지 확인 후 성공을 표시합니다.
