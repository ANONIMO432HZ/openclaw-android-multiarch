...
cmd_ui() {
    ...
    # 1. Native Motor + Token Capture (The Master Wheel 2.0)
    # We run the official binary and capture its output to get the REAL token.
    local RAW_OUTPUT
    RAW_OUTPUT=$(openclaw dashboard 2>&1 || true)
    
    # Clean ANSI and extract token
    local CLEAN_OUTPUT
    ...
    [ -n "$RAW_MATCH" ] && TOKEN="${RAW_MATCH#*token=}"
    ...
    elif [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]] || command -v start &>/dev/null; then
        echo "(Windows)"
        # Note: in plain sh/bash, start is often a shell builtin or needs 'cmd /c start'
        cmd.exe /c start "" "$LOCAL_URL" >/dev/null 2>&1 || start "" "$LOCAL_URL" >/dev/null 2>&1 || true
    else
        echo -e "\n   ${YELLOW}[!]${NC} No automatic opener found. Open manually."
    fi
    ...
    echo ""
    echo -e "${BOLD}3. Remote Access (SSH Tunneling)${NC}"
    echo -e "   Command: ${YELLOW}ssh -N -L 18789:127.0.0.1:18789 ${USER}@${IP}${NC}"
    echo ""
    
    if [ -z "$TOKEN" ]; then
        echo -e "${RED}[WARNING]${NC} Token not detected. Run 'openclaw dashboard' manually for diagnostics."
    else
        echo -e "${DIM}${ITALIC}Session token generated by OpenClaw Core.${NC}"
    fi
    echo ""
}
